...
def ask_ai(question, lang, ctx=""):
    ...
    try:
        ...
        sys = ("Ozbekiston buxgalteriyasi mutaxassisi. Ozbek tilida javob ber." if lang == "uz"
               else "Ты бухгалтер Узбекистана. Отвечай на русском.") + (f" Schyotlar: {ctx}" if ctx else "")
        msg = client.messages.create(model="claude-opus-4-5", max_tokens=1500, system=sys,
...
TOPICS_UZ = ["Buxgalteriya tamoyillari","Aktivlar tasnifi","Passivlar va kapital",
              "Daromad va xarajatlar","Kassa va bank","Asosiy vositalar","Nomoddiy aktivlar",
              "Tovar-moddiy boyliklar","Debitorlik qarzdorligi","Ish haqi hisobi",
              "QQS va soliq","Moliyaviy hisobot","Balans","Foyda va zarar",
              "Moliyaviy tahlil","Audit va nazorat","Budjet hisobi","IFRS standartlari",
              "Sugurta buxgalteriyasi","Elektron buxgalteriya 1C"]
TOPICS_RU = ["Принципы бухучёта","Классификация активов","Пассивы и капитал",
             "Доходы и расходы","Касса и банк","Основные средства","Нематериальные активы",
             "ТМЦ","Дебиторка","Учёт зарплаты","НДС и налоги","Финансовая отчётность",
             "Баланс","Прибыль и убыток","Финансовый анализ","Аудит","Бюджет","МСФО",
             "Бухучёт страховых","1С и электронный учёт"]
...
def gen_lesson(lang, n):
    ...
    try:
        client = anthropic.Anthropic(api_key=ANTHROPIC_API_KEY)
        if lang == "uz":
            p = f"{topic} mavzusida {n}-dars yoz. Tuzilma: 1)Mavzu 2)Nazariya 3)Misol 4)Xulosalar 5)Savol. Emoji ishlat. Ozbek tilida."
        else:
            p = f"Урок {n} по теме: {topic}. Структура: 1)Тема 2)Теория 3)Пример 4)Выводы 5)Вопрос. Используй эмодзи. На русском."
        msg = client.messages.create(model="claude-opus-4-5", max_tokens=2000, messages=[{"role":"user","content":p}])
        return msg.content[0].text, topic
    except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Xush kelibsiz! / Dobro pozhalovat!\n\nTilni tanlang:", reply_markup=InlineKeyboardMarkup(kb))

async def menu(msg, lang, uid, edit=False):
    sub = uid in subscribers
    if lang == "uz":
        txt = f"Bosh menyu\n\nImkoniyatlar:\n- AI buxgalter\n- Schyotlar rejasi\n- Kunlik darslar: {'Obuna bor' if sub else 'Obuna yoq'}\n\nSavol yozing yoki tugma bosing:"
        ...
        txt = f"Glavnoe menyu\n\nFunkcii:\n- AI buxgalter\n- Plan schetov\n- Uroki: {'Podpisan' if sub else 'Ne podpisan'}\n\nNapishite vopros ili nazhite knopku:"
...
async def reload_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    s = reload_cache()
    await m.edit_text(f"Yangilandi! {len(s)} ta." if lang == "uz" else f"Obnovleno! {len(s)} schetov.")
...
async def ai_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sch = get_schyotlar()
    ctx = "; ".join([f"{s['raqam']}: {s['nom']}" for s in sch if any(w.lower() in q.lower() for w in s['nom'].split())][:5])
...
async def btn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d in ("lang_uz", "lang_ru"):
        ...
        subscribers.add(uid)
        await q.message.reply_text("Obuna boldingiz! Har kuni 10:00 da dars keladi." if lang == "uz" else "Podpisalis! Urok kazhdyy den v 10:00.")
        await menu(q.message, lang, uid)
    elif d == "unsub":
        subscribers.discard(uid)
        await q.message.reply_text("Obuna bekor qilindi." if lang == "uz" else "Otpisalis.")
        await menu(q.message, lang, uid)
    elif d == "ai_hint":
        await q.message.reply_text("Savolingizni yozing (masalan: QQS qanday hisoblanadi?)" if lang == "uz" else "Napishite vopros (naprimer: Kak schitat NDS?)")
    elif d == "sch_menu":
        sch = get_schyotlar()
        if not sch:
        ...
        kb = [[InlineKeyboardButton(f"{g} seriya ({len(v)} ta)", callback_data=f"g_{g}")] for g, v in sorted(guruhlar.items())]
        kb.append([InlineKeyboardButton("Orqaga" if lang == "uz" else "Nazad", callback_data="back")])
        await q.edit_message_text(f"Schyotlar: {len(sch)} ta" if lang == "uz" else f"Schety: {len(sch)}", reply_markup=InlineKeyboardMarkup(kb))
    elif d.startswith("g_"):
        g = d[2:]
        sch = get_schyotlar()
        res = [s for s in sch if s["raqam"] and s["raqam"][0].isdigit() and (s["raqam"][0]+"000") == g]
        txt = f"{g} seriyasi {len(res)} ta:\n\n" + "\n".join([f"{s['raqam']} - {s['nom']}" + (f" [{s['tur']}]" if s['tur'] else "") for s in res])
